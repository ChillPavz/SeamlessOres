...
public class SourceBehaviourOreBlock extends DropExperienceBlock {
    ...
    // At 1.20.1 DropExperienceBlock takes (Properties, IntProvider); from 1.21 the order flips.
    public SourceBehaviourOreBlock(BlockBehaviour.Properties properties, IntProvider xp, ResourceLocation sourceId) {
        super(properties, xp);
        this.sourceId = sourceId;
    }

    // At 1.20.1 playerWillDestroy returns nothing; from 1.21 it returns the state.
    @Override
    public void playerWillDestroy(Level level, BlockPos pos, BlockState state, Player player) {
        final Block delegate = source();
        if (delegate != null) {
            try {
                delegate.playerWillDestroy(level, pos, state, player);
                return;
