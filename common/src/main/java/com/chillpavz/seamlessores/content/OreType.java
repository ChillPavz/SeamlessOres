...
public record OreType(String name, String overlay, String deepslateOverlay, String requiredModId,
    ...
    // --- Tech Reborn (Fabric only, 1.21.4 to 1.21.8) -------------------------------------------------
    // XP read from TR's OreDistribution enum, which hands each ore block its range: ruby and sapphire
    // 2-6, every other ore 0. No uranium in this range (5.12.9, 5.13.4 and 5.14.2 all checked).
    // Prefixed where the plain name is taken; block ids are a one-way door, so these names match
    // every other branch.
